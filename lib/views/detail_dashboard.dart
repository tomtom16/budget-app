...
class _DetailDashboardState extends State<DetailDashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    var colChildren = [
      ...
      // Dashbaord headline
      Text(
          isMonthly ? 'Monthly Details: ${currentDate.year}' : 'Yearly Details',
    ...
    colChildren.addAll(buildChart(isMonthly));
  ...
  List<Widget> buildChart(bool isMonthly) {
    if (data.isEmpty) {
      return [
        SizedBox(
            height: 32
  ...
  List<LineChartBarData> _buildSeries() {
    return data.entries
        .where(
            (entry) => visibleSeries.contains(entry.key)) // only visible ones
        .map((entry) {
